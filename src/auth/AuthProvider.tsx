...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadProfile = React.useCallback(async (currentSession:Session) => { setProfileLoading(true); let currentProfile:Profile|null=null; for(let attempt=0;attempt<4&&!currentProfile;attempt+=1){ currentProfile=await supabase.getProfile(currentSession).catch(()=>null); if(!currentProfile&&attempt<3) await sleep(320); } setProfile(currentProfile); setProfileLoading(false); }, []);
