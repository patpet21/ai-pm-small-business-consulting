...
function AuthPanel() {
  const { profile, profileLoading, refresh, session, signOut } = useAuth(); const navigate = useNavigate();
  ...
  const recoverGoogleProfile = async () => { if (!session) return; setBusy(true); setMessage(''); try { await supabase.ensureGoogleProfile(session); await refresh(); navigate('/library'); } catch (error) { const detail = error instanceof Error ? error.message : 'Please try again.'; setMessage(`We could not create your Google Library profile: ${detail}`); } finally { setBusy(false); } };
  const restartGoogle = async () => { await signOut(); await google(); };
  ...
  if (profile || supabase.hasLibraryAccess()) return <div className="library-access-form"><h2>Your Library account is ready.</h2><button className="button primary" onClick={() => navigate('/library')}>Open My Library</button></div>;
  if (session) return <div className="library-access-form"><p className="eyebrow">GOOGLE MEMBER ACCESS</p><h2>{profileLoading ? 'Preparing your Google Library account…' : 'Finish opening your Google Library account'}</h2><p>{profileLoading ? 'We are checking your Google account details.' : 'Your Google sign-in was received. Complete the Library profile setup to open your dashboard.'}</p>{!profileLoading && <><button className="button primary" disabled={busy} onClick={() => void recoverGoogleProfile()}>{busy ? 'Opening your Library…' : 'Open My Library'}</button><button className="button secondary" disabled={busy} onClick={() => void restartGoogle()}>Sign in with Google again</button></>}{message && <p className="form-error">{message}</p>}</div>;
