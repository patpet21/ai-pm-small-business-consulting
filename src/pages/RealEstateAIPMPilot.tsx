...
export function RealEstateAIPMPilot() {
  ...
  const requiredValid = !!report && hasValue(report.workflowDetected) && hasValue(report.mainBottleneck) && safeArray<string>(report.topWorkflowGaps).length > 0 && hasValue(report.first48HourFix) && safeObjectArray<WbsItem>(report.wbsTaskBreakdown).length > 0 && safeObjectArray<PromptItem>(report.aiPromptPack).length > 0 && safeArray<string>(report.sevenDayRoadmap).length > 0;
