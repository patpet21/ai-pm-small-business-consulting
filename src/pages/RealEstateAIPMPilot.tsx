...
export function RealEstateAIPMPilot() {
  ...
  const requiredValid = !!report && hasValue(report.workflowDetected) && hasValue(report.mainBottleneck);
  ...
  if (submitted) {
    ...
    return (
      <section className="section-shell pilot-success">
        ...
        {report && (!safeArray<string>(report.topWorkflowGaps).length || !hasValue(report.first48HourFix) || !safeObjectArray<WbsItem>(report.wbsTaskBreakdown).length || !safeObjectArray<PromptItem>(report.aiPromptPack).length || !safeArray<string>(report.sevenDayRoadmap).length) && (<p className="partial-note">Some advanced sections were not returned by AI for this submission. You can request a human-reviewed report for the complete version.</p>)}
        ...
        <div className="resource-card snapshot-card">
          ...
          <div className="snapshot-grid report-section-gap"><div className="snapshot-full"><h3>Top 3 Workflow Gaps</h3>{topGaps.length ? <ul>{topGaps.map((g) => <li key={g}>{g}</li>)}</ul> : <p>This will be reviewed in the human-reviewed report.</p>}</div></div>
          <div className="snapshot-grid report-section-gap"><div className="snapshot-full"><h3>{safeText(fix.title, 'Your First 48-Hour Fix')}</h3>{hasValue(report.first48HourFix) ? <><p>{safeText(fix.description)}</p>{hasValue(fix.trackerName) && <p><strong>Tracker:</strong> {safeText(fix.trackerName)}</p>}{safeArray<string>(fix.columns).length > 0 && <><p><strong>Columns:</strong></p><ul>{safeArray<string>(fix.columns).map((c) => <li key={c}>{c}</li>)}</ul></>}{safeArray<string>(fix.statuses).length > 0 && <><p><strong>Statuses:</strong></p><ul>{safeArray<string>(fix.statuses).map((s) => <li key={s}>{s}</li>)}</ul></>}</> : <p>This will be reviewed in the human-reviewed report.</p>}</div></div>
          <div className="snapshot-grid report-section-gap"><div className="snapshot-full"><h3>WBS-Based Workflow Breakdown</h3>{wbs.length ? <div className="rows-list">{wbs.map((i,idx)=><div className="row-card" key={idx}><p><strong>Task:</strong> {safeText(i.taskName)}</p><p><strong>Owner Type:</strong> {safeText(i.ownerType)}</p><p><strong>Output:</strong> {safeText(i.output)}</p><p><strong>Acceptance Criteria:</strong> {safeText(i.acceptanceCriteria)}</p></div>)}</div> : <p>This will be reviewed in the human-reviewed report.</p>}</div></div>
          <div className="snapshot-grid report-section-gap"><div className="snapshot-full"><h3>Copy/Paste AI Prompt Pack</h3>{prompts.length ? <div className="rows-list">{prompts.map((p,idx)=><div className="row-card" key={idx}><p><strong>{safeText(p.title, `Prompt ${idx+1}`)}</strong></p><p>{safeText(p.prompt)}</p></div>)}</div> : <p>This will be reviewed in the human-reviewed report.</p>}</div></div>
          ...
          <div className="snapshot-grid report-section-gap"><div className="snapshot-full"><h3>7-Day Implementation Roadmap</h3>{roadmap.length ? <ol className="roadmap-list">{roadmap.map((r,i)=><li key={`${i}-${r}`}>{r}</li>)}</ol> : <p>This will be reviewed in the human-reviewed report.</p>}</div></div>
