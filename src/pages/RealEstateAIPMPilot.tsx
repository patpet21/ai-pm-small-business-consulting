    ...
    return (
      <section className="section-shell pilot-success">
        ...
        <div className="resource-card snapshot-card">
          ...
          {scarf.length > 0 && <div className="snapshot-grid report-section-gap"><div className="snapshot-full"><h3>SCARF Trust & AI Adoption Check</h3><div className="scarf-grid">{scarf.map((s,idx)=><div className="scarf-card" key={idx}><h4>{safeText(s.domain)}</h4><p><strong>Risk:</strong> {safeText(s.risk)}</p><p><strong>Recommendation:</strong> {safeText(s.recommendation)}</p></div>)}</div></div></div>}
