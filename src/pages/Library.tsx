...
function Account() { const { profile, session, refresh, signOut } = useAuth(); const [username, setUsername] = React.useState(profile?.username ?? ''); const [email, setEmail] = React.useState(profile?.email ?? ''); const [marketing, setMarketing] = React.useState(profile?.marketing_consent ?? false); const [message, setMessage] = React.useState(''); return <div className="library-account"><p className="eyebrow">MEMBER PROFILE</p><h1>My Account</h1><label>Username<input value={username} onChange={(event) => setUsername(event.target.value)} /></label><label>Email address<input value={email} onChange={(event) => setEmail(event.target.value)} /></label><label className="check"><input type="checkbox" checked={marketing} onChange={(event) => setMarketing(event.target.checked)} />Receive occasional Library updates.</label><button className="button primary" onClick={async () => { if (session && profile) { await supabase.saveProfile(session, { username, email, marketing_consent: marketing, privacy_accepted_at: profile.privacy_accepted_at }); await refresh(); setMessage('Profile updated.'); } }}>Save profile</button>{message && <p>{message}</p>}<div className="library-account-note">These details are profile information, not password replacements. No authentication email is sent. This session belongs to the current browser.</div><button className="button secondary" onClick={() => void signOut()}>Sign out of this browser</button></div>; }
...
export function Library() {
  ...
  if (loading || !session || !profile) return <div className="library-loading">Preparing your Library…</div>;
