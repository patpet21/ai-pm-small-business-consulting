...
export function Library() {
  const { loading, profileLoading, session, profile, signOut } = useAuth(); const navigate = useNavigate(); const [tab, setTab] = React.useState<Tab>('Overview'); const [items, setItems] = React.useState<Resource[]>(fallbackCatalog); const [saved, setSaved] = React.useState<string[]>([]); const [query, setQuery] = React.useState('');
  ...
  if (loading || !session || profileLoading) return <div className="library-loading">Preparing your Library…</div>;
  if (!profile) return <div className="library-loading">Finishing your Library profile…</div>;
